...
- [ ] **VOT-01**: Ties always resolve — `AGUARDANDO_DESEMPATE` reaches a terminal status even when the chefe already voted during the normal phase (decide mechanism: chefe `changeMyVote` allowed in tiebreak status, or chefe's regular vote excluded from tie-break eligibility); every `status ===` guard grepped before changing
